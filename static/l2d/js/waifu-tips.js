// 兜底定义 loadlive2d，避免未定义报错
if (typeof loadlive2d === 'undefined') {
    window.loadlive2d = function(canvasId, configStr) {
        console.log("等待 live2d.js 加载，暂存请求：", configStr);
        // 暂存请求参数
        window._l2dQueue = window._l2dQueue || [];
        window._l2dQueue.push({canvasId: canvasId, configStr: configStr});
    };
    // 监听 live2d.js 加载完成
    var checkLive2d = setInterval(function() {
        if (typeof window._realLoadlive2d !== 'undefined') {
            // 执行暂存的请求
            window._l2dQueue.forEach(function(item) {
                window._realLoadlive2d(item.canvasId, item.configStr);
            });
            clearInterval(checkLive2d);
        }
    }, 100);
    // 覆盖 loadlive2d 的赋值，捕获真实函数
    Object.defineProperty(window, 'loadlive2d', {
        set: function(fn) {
            window._realLoadlive2d = fn;
        },
        get: function() {
            return window._realLoadlive2d || function() {};
        }
    });
}

// 封装加载模型的函数（核心修正）
function loadL2DModel(p, id) {
    // 拼接API地址（去掉.php，用Flask接口）
    var apiUrl = $(".l2d_xb").attr("data-api") + "?p=" + p;
    if (id !== undefined) {
        apiUrl += "&id=" + id;
    }
    // 请求配置并加载模型
    $.getJSON(apiUrl, function(config) {
        loadlive2d('live2d', JSON.stringify(config));
    }).fail(function(err) {
...
// 仅保留核心逻辑，删除所有自定义等待/封装代码
var model_p = 22,m22_id = m33_id = 0;

// 初始加载（直接调用，不做任何封装）
loadlive2d('live2d', $('.l2d_xb').attr('data-api') + '?p=33&id=0');

// 切换角色
$('.waifu-tool .drivers-license-o').click(function(){
    if(model_p===22){
        loadlive2d('live2d',$(".l2d_xb").attr("data-api")+'?p=22&id='+m22_id);
        model_p = 33;
        showMessage('33有点累了，现在该我上场了',4000)
    }else{
        loadlive2d('live2d',$(".l2d_xb").attr("data-api")+'?p=33&id='+m33_id);
        model_p = 22;
        showMessage('我叒回来了！',4000)
    }
});
...
// 初始加载（恢复原逻辑）
//loadlive2d('live2d',$(".l2d_xb").attr("data-api")+'?p=33');
...
$("#live2d").mouseover(function(){
    msgs = ["你要干嘛呀？","鼠…鼠标放错地方了！","喵喵喵？","萝莉控是什么呀？"];
